...
def mobilenet_v3_small(inputs, multiplier=cfgs.MBV3_SHRINK, is_training=False, reuse=None):
    ...
    input_size = inputs.get_shape().as_list()[0:-1]
    assert ((input_size[1] % 32 == 0) and (input_size[2] % 32 == 0))
    ...
    with tf.variable_scope("MobilenetV3_small", reuse=reuse):
        ...
        x = _fully_connected_layer(x, units=cfgs.NUM_CLASS + 1, name='cls_out', reuse=None)
        x = tf.reshape(x, [input_size[0], 1, cfgs.NUM_CLASS + 1])
        end_points['cls_prob'] = x

    return end_points['bneck0'], end_points['bneck2'], end_points['bneck7'], end_points['bneck10'], end_points['cls_prob']