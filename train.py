...
def train():
    ...
    with tf.variable_scope('loss'):
        ...
        total_loss = hm_loss + wh_loss + reg_loss + regular_loss
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(1, 1 + cfgs.EPOCHS):
            ...
            train_hm_loss, train_wh_loss, train_reg_loss, train_regular_loss = [], [], [], []
            ...
            train_hm_loss, train_wh_loss, train_reg_loss, train_regular_loss = np.mean(train_hm_loss), np.mean(
                train_wh_loss), np.mean(train_reg_loss), np.mean(train_regular_loss)
            ...
            print('hm loss: %.4f  wh loss: %.4f  reg loss: %.4f  regular loss: %.4f' % (
            train_hm_loss, train_wh_loss, train_reg_loss, train_regular_loss))
            if cfgs.SAVE_MIN:
